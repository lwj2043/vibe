...
[사전 자가검증 체크리스트 — 출력 전에 반드시 통과시킬 것]
코드를 내기 전 아래 모든 항목을 스스로 확인하세요. 하나라도 미충족이면 통과할 때까지 다시 작성하세요.
런타임/구문:
- 닫히지 않은 태그·괄호·따옴표가 없고, 정의되지 않은 변수·함수 호출이 없다.
- 누락된 import / 존재하지 않는 경로 참조가 없다.
- 콘솔 에러를 유발할 요소(존재하지 않는 DOM id 접근, 초기 null 접근, 잘못된 JSON 파싱 등) 가 없다.
렌더 가능성(프론트):
- 첫 화면이 비어 있거나 완전한 검은 화면이 되지 않는다 — body 에 최소 한 개 이상의 보이는 콘텐츠가 초기 렌더에서 표시된다.
- 외부 이미지가 필요한 경우 로드 실패 시에도 레이아웃이 깨지지 않는다 (대체 배경/플레이스홀더 처리).
- 명세서 `features` 각 항목에 대응하는 **실제 UI 요소**(버튼/입력/리스트 등) 가 초기 마크업에 존재한다.
명세 준수:
- `features` 전부가 코드에 반영되어 있다 (acceptance_criteria 를 만족).
- `files` 목록 전부를 출력했고, 경로가 정확하다.
- `constraints` 전부 위반 없이 준수 (특히 "단일 HTML 파일" 같은 파일 구조 제약).
플레이스홀더 금지:
- 본문·주요 UI 자리에 Lorem ipsum / TODO / FIXME / "여기에 내용" 같은 플레이스홀더가 남아 있지 않다 (짧은 샘플 라벨은 허용).

위 체크리스트는 "개선 가이드" 가 아니라 **제출 전 필수 통과선** 입니다. 통과 전에는 출력하지 마세요.""".strip()
...
{
  "ok": true | false,
  "blocking_issues": ["출력/실행을 막는 심각한 문제만 한 줄씩"],
  "minor_issues": ["개선하면 좋은 사소한 지적 한 줄씩 (없으면 빈 배열)"],
  "fix_instructions": "blocking_issues 를 해결하기 위한 구체적 지시. blocking_issues 가 비어 있으면 반드시 빈 문자열."
}

[판정 원칙]
- **기본값은 ok=true**. 아래 "블로킹 기준" 에 해당하는 문제가 **하나라도** 있을 때만 ok=false.
- 사소한 개선점(네이밍, 스타일, 미세한 UX 개선 등)은 minor_issues 에만 넣고 ok 판정에 영향을 주지 않습니다.
- blocking_issues 와 minor_issues 를 엄격히 분리하세요. "더 좋을 수 있는" 항목을 blocking_issues 에 넣으면 안 됩니다.
...
위 블로킹 기준 중 어느 것에도 해당하지 않으면 {"ok": true, "blocking_issues": [], "minor_issues": [...개선점...], "fix_instructions": ""} 를 출력하세요.
ok=false 로 판정할 때는 blocking_issues 에 기능 ID / 파일명 / 에러 위치를 명시하고, fix_instructions 에 blocking_issues 만 겨냥한 구체적 수정 방향을 적으세요. minor_issues 를 fix_instructions 에 섞지 마세요.""".strip()
...
{
  "ok": true | false,
  "blocking_issues": ["출력/실행을 막는 심각한 문제만 한 줄씩 — '코드:' 또는 '디자인:' 접두어"],
  "minor_issues": ["개선하면 좋은 사소한 지적 한 줄씩 (없으면 빈 배열)"],
  "fix_instructions": "blocking_issues 를 해결하기 위한 구체적 지시. blocking_issues 가 비어 있으면 반드시 빈 문자열."
}
...
[판정 원칙]
- **기본값은 ok=true**. 아래 "블로킹 기준" 에 해당하는 문제가 **하나라도** 있을 때만 ok=false.
- 사소한 개선점(미세한 여백·색상 조정, 시각 계층 강화 등)은 minor_issues 에만 넣고 ok 판정에 영향 없음.
- blocking_issues 와 minor_issues 를 엄격히 분리하세요. "더 좋을 수 있는" 디자인 개선은 blocking 이 아닙니다.
...
ok=false 로 판정할 때는 blocking_issues 에 기능 ID / 파일명 / 에러 원문을 명시하고, fix_instructions 에 blocking_issues 만 겨냥한 구체적 수정 방향을 적으세요. minor_issues 를 fix_instructions 에 섞지 마세요."""
...
[첨부 화면(있는 경우)]
메시지에 스크린샷이 첨부되어 있으면, 블로킹 문제가 실제 화면에서 어떻게 나타나는지 확인한 뒤 수정하세요.

규칙:
- 위 블로킹 문제만 집중적으로 해결하세요. 지적되지 않은 부분은 변경하지 마세요.
- minor 개선(스타일 취향, 미세 여백 등) 을 이 수정 단계에서 함께 하지 마세요 — 새 버그를 유발합니다.
- 수정된 **전체 코드**를 파일 블록 형식 그대로 다시 출력하세요.
"""
