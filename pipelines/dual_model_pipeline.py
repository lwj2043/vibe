...
class Pipeline:
    ...
    def _fix_code(
        ...
        del messages  # coder model은 history 미지원
        # 명세·이슈·지시만 번역 (코드 자체는 원문 유지)
        ko_meta = (
            f"[기술 명세서]\n{json.dumps(spec, ensure_ascii=False, indent=2)}\n\n"
            f"[문제점]\n{chr(10).join(f'- {i}' for i in issues) or '(없음)'}\n\n"
            f"[수정 지시]\n{fix_instructions or '검토 결과를 참고하여 수정'}"
        )
        zh_meta = self._translate_for_coder(ko_meta)
        prompt = (
            f"{zh_meta}\n\n"
            f"[上次生成的代码]\n{previous_code}\n\n"
            "请根据上述指示，重新输出修改后的完整代码。"
        )
        yield from self._stream_coder(system=FIX_CODER_SYSTEM_PROMPT_ZH, user=prompt)
    ...
    def _run_spec_code_review_loop(
        ...
        while attempt < hard_cap:
            ...
            if attempt == 1:
                ...
                gen = self._fix_code(
                    spec=spec,
                    previous_code=code_zh,  # Qwen은 중국어 코드를 받아 수정
            ...
            try:
                review, _mode = self._review_code(spec, code_ko)
            except Exception:
        ...
        if final_note:
            yield final_note
        yield from utils.stream_text(code_ko)
